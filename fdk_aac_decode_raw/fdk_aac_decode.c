...
#define AAC_LC_SBR_FILENAME_PATTERN "./AAC-LC-SBR/dump_%d.raw"
#define PCM_OUTPUT_FILE_NAME "./output.pcm"

int g_decode_aac_profile = AOT_SBR;
...
void prepare_aac_raw_buf_list()
{
  ...
  while (1)
  {
    ...
    else if (g_decode_aac_profile == AOT_SBR)
    {
      sprintf(dfilename, AAC_LC_SBR_FILENAME_PATTERN, raw_buf_total_cout);
    }    
    else
    {
      exit(1);
...
int main()
{
...
#if 1
    UCHAR conf[] = {0x11, 0x90, 0x00, 0x07};  //AAL-LC 44100kHz STEREO
    UCHAR* conf_array[1] = { conf };
    UINT length = 4;
#else
  ...
  {
    UCHAR conf[] = {0x49, 0x90}; //AAL-LC-SBR 48kHz 2 channels
    UCHAR *conf_array[1] = {conf};
    UINT length = 2;
    err = aacDecoder_ConfigRaw(decoder, conf_array, &length);
    assert(!err);    
